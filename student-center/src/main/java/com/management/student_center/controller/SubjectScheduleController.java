...
@RestController
@RequestMapping("/v1/api") // giữ routes tương tự: /session/:sessionId và /schedule/:subjectId
public class SubjectScheduleController {
    ...
    public ResponseEntity<?> deleteSession(@PathVariable("sessionId") Long sessionId) {
        try {
            ...
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(Map.of("message", "Có lỗi xảy ra khi xoá buổi học", "error", e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> createSubjectSchedule(@RequestBody CreateSubjectScheduleRequest req) {
        try {
            ...
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(Map.of("message", "Có lỗi xảy ra khi tạo lịch học", "error", e.getMessage()));
        }
    }
