...
public class SubjectScheduleService {
    ...
    public Map<String, Object> deleteSession(Long sessionId) {
        ...
        activityLogService.log(currentUser, ActivityActionType.DELETE, ActivityTargetType.SCHEDULE, 
                sessionId, description, meta);
    ...
    public Map<String, Object> createSubjectSchedule(CreateSubjectScheduleRequest req) {
        ...
        String meta = String.format(
                ...
                room != null ? room.getId() : "null", escapeJson(roomName),
                req.getStartDate(), req.getEndDate() != null ? req.getEndDate() : "null", sessions.size());
        
        activityLogService.log(currentUser, ActivityActionType.CREATE, ActivityTargetType.SCHEDULE, 
                schedule.getId(), description, meta);
    ...
    public SessionDTO addManualSession(ManualSessionRequest req) {
        ...
        String roomName = room != null ? room.getName() : "chưa có phòng";
        String description = String.format("đã thêm buổi học thủ công cho môn học %s (Khối %s) vào ngày %s (%s - %s) tại phòng %s",
        ...
        String meta = String.format(
                ...
                req.getSessionDate(), req.getStartTime(), req.getEndTime(),
                room != null ? room.getId() : "null", escapeJson(roomName),
                req.getScheduleId() != null ? req.getScheduleId() : "null");
        
        activityLogService.log(currentUser, ActivityActionType.CREATE, ActivityTargetType.SCHEDULE, 
                session.getId(), description, meta);
    ...
    public SessionDTO updateSession(Long sessionId, ManualSessionRequest req) {
        ...
        if (changes.isEmpty()) {
            ...
            description = String.format("đã cập nhật buổi học của môn học %s (Khối %s): %s", 
                    subjectName, subjectGrade, String.join(", ", changes));
        }
        ...
        String meta = String.format(
                ...
                sessionId, subjectId, escapeJson(subjectName),
                oldDate, oldStart, oldEnd, escapeJson(oldRoomName), oldStatus,
                newDate, newStart, newEnd, escapeJson(newRoomName), newStatus);
        
        activityLogService.log(currentUser, ActivityActionType.UPDATE, ActivityTargetType.SCHEDULE, 
                sessionId, description, meta);
